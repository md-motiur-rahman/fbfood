    ...
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <main className="mx-auto max-w-5xl px-4 sm:px-6 py-8">
        <header className="mb-6">
          ...
          <p className="mt-2 text-sm text-slate-600">Review your items, adjust quantities, and provide your contact details. We will reach out with pricing and freight details.</p>
        </header>
        ...
        <section className="rounded-lg border border-sky-200 bg-white overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {items.length === 0 ? (
                  <tr><td colSpan={4} className="px-3 py-6 text-center text-slate-600">Your quote is empty. Browse products and add items to quote.</td></tr>
                ) : (
                  items.map((it, index) => (
                    <tr key={`${it.barcode}-${index}`} className="border-t border-sky-100">
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-3">
                          {it.picture ? (<img src={it.picture} alt={it.productname} className="h-10 w-10 rounded object-cover" />) : (<div className="h-10 w-10 rounded bg-slate-100" />)}
                      ...
                      <td className="px-3 py-2">
                        <input
                          ...
                          className="w-24 rounded border border-sky-200 px-2 py-1"
                        />
                      </td>
        ...
        {/* Submission form */}
        <section className="mt-6 rounded-lg border border-sky-200 bg-white p-4">
          ...
          <form onSubmit={onSubmit} className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
            <label className="text-sm">Name
              <input value={form.name} onChange={(e) => setForm((s) => ({ ...s, name: e.target.value }))} required className="mt-1 w-full rounded border border-sky-200 px-3 py-2" />
            </label>
            <label className="text-sm">Email
              <input value={form.email} type="email" onChange={(e) => setForm((s) => ({ ...s, email: e.target.value }))} required className="mt-1 w-full rounded border border-sky-200 px-3 py-2" />
            </label>
            <label className="text-sm">Company
              <input value={form.company} onChange={(e) => setForm((s) => ({ ...s, company: e.target.value }))} className="mt-1 w-full rounded border border-sky-200 px-3 py-2" />
            </label>
            <label className="text-sm">Phone
              <input value={form.phone} onChange={(e) => setForm((s) => ({ ...s, phone: e.target.value }))} className="mt-1 w-full rounded border border-sky-200 px-3 py-2" />
            </label>
            <label className="text-sm sm:col-span-2">Notes
              <textarea value={form.notes} onChange={(e) => setForm((s) => ({ ...s, notes: e.target.value }))} rows={4} className="mt-1 w-full rounded border border-sky-200 px-3 py-2" />
            </label>
            <div className="sm:col-span-2 flex items-center justify-end gap-2">
              <button type="button" onClick={clear} disabled={items.length === 0} className="rounded border border-sky-200 px-3 py-2 text-sm disabled:opacity-50">Clear</button>
              <button type="submit" disabled={disabled} className="rounded bg-sky-500 px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-sky-400 disabled:opacity-50">
