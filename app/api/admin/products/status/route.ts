...
export async function PUT(req: Request) {
  ...
  if (raw !== "AVAILABLE" && raw !== "UNAVAILABLE") return NextResponse.json({ error: "Invalid status" }, { status: 400 });
  ...
  try {
    ...
    const msg = (err as { message?: string })?.message || "Bulk status update failed";
