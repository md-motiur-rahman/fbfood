...
export async function PUT(req: Request) {
  ...
  const b = body as { ids?: unknown; promotion_type?: unknown };
  ...
  const raw = typeof b.promotion_type === "string" ? b.promotion_type.toUpperCase() : "";
  const promotion_type = raw === "MONTHLY" || raw === "SEASONAL" ? raw : null; // null clears promotion
  ...
  try {
    ...
    const sql = `UPDATE products SET promotion_type = ? WHERE id IN (${placeholders})`;
    const params = [promotion_type, ...ids];
    ...
    const msg = (err as { message?: string })?.message || "Bulk update failed";
