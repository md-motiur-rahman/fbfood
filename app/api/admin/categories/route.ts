...
export async function POST(req: Request) {
  ...
  try {
    ...
    const [res] = await pool.execute<ResultSetHeader>(
      `INSERT INTO categories (name, slug, picture, picture_key, mime_type, size_bytes, width, height)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    ...
    const code = err?.code;
    const msg = code === "ER_DUP_ENTRY" ? "A category with this slug already exists." : (err?.message || "Insert failed");
