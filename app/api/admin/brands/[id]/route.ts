...
export async function PUT(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [res] = await pool.execute<ResultSetHeader>(
      `UPDATE brands SET name=?, slug=?, picture=?, picture_key=?, mime_type=?, size_bytes=?, width=?, height=? WHERE id=?`,
    ...
    const code = err?.code;
    const msg = code === "ER_DUP_ENTRY" ? "A brand with this slug already exists." : (err?.message || "Update failed");
