...
async function ensureUniqueSlug(pool: Pool, base: string): Promise<string> {
  ...
  // check existence
  while (true) {
    const [rows] = await pool.query<RowDataPacket[]>("SELECT COUNT(*) as cnt FROM brands WHERE slug = ?", [slug]);
    const cnt = Number(((rows[0] as RowDataPacket) as { cnt?: number }).cnt ?? 0);
    if (cnt === 0) return slug;
    suffix += 1;
    slug = `${base || "brand"}-${suffix}`;
  }
}
...
async function saveImageFromSource(src: string): Promise<{ publicPath: string } | null> {
  ...
  // - http(s) URL (download)
  // - /public path (/images/foo.jpg)
  const baseDir = path.join(process.cwd(), "public", "uploads", "brands");
  ...
  if (src.startsWith("data:image/")) {
    ...
    return { publicPath: "/uploads/brands/" + name };
  }
  ...
  if (src.startsWith("/")) {
    ...
    try {
      ...
      return { publicPath: src }; // fallback to existing public path
    }
  }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        const [res] = await pool.execute<ResultSetHeader>(
          `INSERT INTO brands (name, slug, picture, picture_key) VALUES (?, ?, ?, ?)`,
