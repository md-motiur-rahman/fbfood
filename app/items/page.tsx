...
export default function AllItemsPage() {
  ...
  const searched = useMemo(() => {
    ...
    // exact match by product name or barcode within current category scope
    const exact = filteredByCategory.filter(
      (p) => p.productname.toLowerCase() === q || p.outerbarcode.toLowerCase() === q
    );
    if (exact.length) return exact;

    // category query: if q matches category slug or name, show all items in that category
    const catMatch = inventoryCategories.find(
      (c) => c.slug.toLowerCase() === q || c.name.toLowerCase() === q
    );
    if (catMatch) {
      // Respect the active category filter if user selected one.
    ...
    // similar items: name contains q or category name contains q
    ...
    // fallback: return current category scope
    return filteredByCategory;
  ...
  useEffect(() => {
    // Reset to first page on filter/sort/pageSize change
    setPage(1);
  ...
  // Initialize category filter from query if it matches a category
  useEffect(() => {
