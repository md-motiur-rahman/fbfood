          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} className="px-3 py-6 text-center text-slate-600">No brands found</td></tr>
              ) : (
                items.map((b) => (
                  <tr key={b.id} className="border-t border-sky-100 hover:bg-sky-50/50">
                    <td className="px-3 py-2 font-medium">{b.name}</td>
                    <td className="px-3 py-2 font-mono text-xs">{b.slug}</td>
                    <td className="px-3 py-2">{b.picture ? <img src={b.picture} alt={b.name} className="h-10 w-10 object-cover rounded" /> : <span className="text-slate-500">—</span>}</td>
                    <td className="px-3 py-2">{new Date(b.created_at).toLocaleDateString()}</td>
                    <td className="px-3 py-2">
                      <div className="flex items-center gap-2">
                        <button onClick={() => onEditStart(b)} className="rounded border border-sky-200 px-2 py-1 text-xs hover:bg-sky-50">Edit</button>
